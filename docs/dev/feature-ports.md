...
| Chapter content pipeline | `reikai/novel/content/` (`5607fa31b`) | 2026-09-06 @ `fe54f05ae` | Their `shared/ContentPipeline` + `ContentConfig` + `HtmlUtils` + `RegexReplacementsProcessor` and `util/TextSplitter`, re-typed onto `NovelPreferences` with `ln_reader_*` keys. Runs inside `NovelChapterTextLoader` so both readers get it. Their `data-tsundoku-*` HTML markers are renamed `data-reikai-*`, which the WebView port must match. Their theme resolver was not taken: `resolvedForSystemTheme` already answers it. Regex replacements ship without their rule editor, markdown ships unreachable (no source produces a `.md` chapter URL). |
